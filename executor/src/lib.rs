...
native_executor_instance!(
	pub Executor,
	dothereum_runtime::api::dispatch,
	dothereum_runtime::native_version
);
...
mod tests {
	...
	use dothereum_runtime::impls::LinearWeightToFee;
	use dothereum_primitives::{Balance, Hash, BlockNumber};
	...
	const BLOATY_CODE: &[u8] = dothereum_runtime::WASM_BINARY_BLOATY;
	...
	fn new_test_ext(code: &[u8], support_changes_trie: bool) -> TestExternalities<Blake2Hasher> {
		let mut ext = TestExternalities::new_with_code(
			code,
			dothereum_testing::genesis::config(support_changes_trie, Some(code)).build_storage().unwrap(),
		);
...
"#;
	...
	fn should_import_block_with_test_client() {
		use dothereum_testing::client::{ClientExt, TestClientBuilderExt, TestClientBuilder, consensus::BlockOrigin};

		let client = TestClientBuilder::new().build();
		let block1 = changes_trie_block();
		let block_data = block1.0;
		let block = dothereum_primitives::Block::decode(&mut &block_data[..]).unwrap();
	...
	fn block_weight_capacity_report() {
		...
		use dothereum_primitives::Index;

...
impl<C: Get<Balance>> Convert<Weight, Balance> for LinearWeightToFee<C> {
	fn convert(w: Weight) -> Balance {
		// dothereum-node a weight of 10_000 (smallest non-zero weight) to be mapped to 10^7 units of
		// fees, hence:
	...
	fn empty_chain_simulation() {
		...
		run_with_system_weight(block_weight, || {
			...
			assert!(iterations > 50_000, "This assertion is just a warning; Don't panic. \
				Current dothereum nodes are configured with a _slow adjusting fee_ mechanism. \
				Hence, it is really unlikely that fees collapse to zero even on an empty \
				chain in less than at least of couple of thousands of empty blocks. But this \
